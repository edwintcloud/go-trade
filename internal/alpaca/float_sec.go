...
const (
	...
	secUserAgent = "Ingrediscan support@ingrediscan.app"
...
func (fs *FloatStore) fetchFromSEC(ctx context.Context) (int, error) {
	...
	// Step 3: Merge recent quarters from oldest to newest so newer filings win.
	anyData := false
	for i := len(periods) - 1; i >= 0; i-- {
		...
		if total == 0 {
			log.Printf("float-store: SEC frames %s: no data", period)
			continue
		}

		anyData = true
		log.Printf("float-store: SEC frames %s: %d unique symbols so far", period, fs.Len())
	}
